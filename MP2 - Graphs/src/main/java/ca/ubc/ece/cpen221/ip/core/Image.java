...
import ca.ubc.ece.cpen221.ip.mp.DFTOutput;
...
public final class Image implements ActionListener {
    ...
    public static void main(String[] args) {
        Image image = new Image("resources/custom-tests/TestTextImage3-45deg.png");
        ...
        Rectangle middleRectangle = new Rectangle(expectedImage.width() / 3,
            expectedImage.height() / 3, expectedImage.width() * 2 / 3,
            expectedImage.height() * 2 / 3);
        ...
        for (int col = 0; col < width(); col++) {
            for (int row = 0; row < height(); row++) {
                if (this.getRGB(col, row) != that.getRGB(col, row)) {
                    System.out.println("Row: " + row + " and col: " + col + " differ");
                    return false;
                }
            }
        }
